 ...
 */
export default (checkCSS, attrName, element, reverse, expectedValue) => {
    var _a;
    ...
    if (attrName.match(/(font-size|line-height|display|color|font-weight)/)) {
        attributeValue = (_a = attributeValue.value) !== null && _a !== void 0 ? _a : '';
    }
    attributeValue = attributeValue.toString();
    if (reverse) {
        failMessage(() => expect(attributeValue).not.toBe(expectedValue), `${attrType} "${attrName}" of the element "${element}" should not match "${expectedValue}"`);
    }
    else {
        failMessage(() => expect(attributeValue).toBe(expectedValue), `${attrType} "${attrName}" of the element "${element}" should match "${expectedValue}", but is "${attributeValue}"`);
    }
};
