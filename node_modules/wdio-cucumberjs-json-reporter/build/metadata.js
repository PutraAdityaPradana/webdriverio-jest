...
function determineMetadata(data) {
  ...
  const metadata = currentConfigCapabilities['cjson:metadata'] || {}; // When an app is used to test
...
function determineDeviceName(metadata, currentConfigCapabilities) {
  return metadata.device || currentConfigCapabilities.deviceName || _constants.NOT_KNOWN;
}
/**
...
function determinePlatformName(metadata, currentCapabilities) {
  const currentPlatformName = currentCapabilities.platformName ? currentCapabilities.platformName.includes('mac') ? 'osx' : currentCapabilities.platformName.includes('windows') ? 'windows' : currentCapabilities.platformName : _constants.NOT_KNOWN;
...
function determinePlatformVersion(metadata) {
  return metadata && metadata.platform && metadata.platform.version ? metadata.platform.version : _constants.NOT_KNOWN;
}
/**
...
function determineBrowserData(currentCapabilities, currentConfigCapabilities, metadata) {
  ...
  const browserVersion = currentCapabilities.version || currentConfigCapabilities.browserVersion || (metadata && metadata.browser && metadata.browser.version ? metadata.browser.version : 'No metadata.browser.version available');
  return {
    browser: {
      name: browserName,
