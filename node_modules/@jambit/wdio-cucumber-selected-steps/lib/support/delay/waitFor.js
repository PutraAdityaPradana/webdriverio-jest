...
const STATE_HANDLERS = {
    'be checked': (element, timeout, reverse) => browser.waitUntil(() => element().isSelected() !== reverse, {
        timeout,
        timeoutMsg: `Element "${element}" ${reverse ? 'did not get checked within' : 'was still checked after'} ${timeout}ms`,
    }),
    'be selected': (element, timeout, reverse) => browser.waitUntil(() => element().isSelected() !== reverse, {
        timeout,
        timeoutMsg: `Element "${element}" ${reverse ? 'did not get selected within' : 'was still selected after'} ${timeout}ms`,
    }),
    'be enabled': (element, timeout, reverse) => element().waitForEnabled({ timeout, reverse }),
    'be displayed': (element, timeout, reverse) => element().waitForDisplayed({ timeout, reverse }),
    'have a text': (element, timeout, reverse) => browser.waitUntil(() => (getText(element()) !== '') !== reverse, {
        timeout,
        timeoutMsg: `Element "${element}" ${reverse ? 'still had text' : 'still had no text'} after ${timeout}ms`,
    }),
    'match the text': (element, timeout, reverse, value) => browser.waitUntil(() => (getText(element()) === value) !== reverse, {
        timeout,
        timeoutMsg: `Text of the element "${element}" did not change within ${timeout}ms ${reverse ? 'from' : 'to'} "${value}"`,
    }),
    'contain the text': (element, timeout, reverse, value) => browser.waitUntil(() => (getText(element()).includes(value || '')) !== reverse, {
        timeout,
        timeoutMsg: `Text of the element "${element}" did not change within ${timeout}ms to ${reverse ? 'not contain' : 'contain'} "${value}"`,
    }),
    'have a value': (element, timeout, reverse) => browser.waitUntil(() => (element().getValue() !== '') !== reverse, {
        timeout,
        timeoutMsg: `Element "${element}" ${reverse ? 'still had a value' : 'still had no value'} after ${timeout}ms`,
    }),
    'match the value': (element, timeout, reverse, value) => browser.waitUntil(() => (element().getValue() === value) !== reverse, {
        timeout,
        timeoutMsg: `Value of the element "${element}" did not change within ${timeout}ms ${reverse ? 'from' : 'to'} "${value}"`,
    }),
    exist: (element, timeout, reverse) => element().waitForExist({ timeout, reverse }),
};
 ...
 */
export default (timeout, element, reverse, state, value) => {
    if (state) {
